...
router.get("/applicant/me",auth,
    async (req, res) => {
        ...
        try {
            ...
            const existingUser = await Applicant.findOne({ email:req.session.userid })
...
router.put("/applicant",
    async (req, res) => {
        ...
        try {
            //Check if email is existant?
            const existingUser = await Applicant.findOneAndUpdate({email:req.session.userid},req.body)
